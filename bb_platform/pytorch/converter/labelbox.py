...
def converter(path, dirs=[], instance=None):
    ...
    """
    try:
        logger.info('Run Labelbox converter')
        ...
        for idx, sub_folder in enumerate(sub_folders):
            ...
            for i, item in enumerate(annotation_data):
                ...
                # Image label
                label = item['Label']
                label_classification = label['classifications'][0]
                answer = label_classification['answer']
                # Read the title to align LABEL_CLASS_MAP (capitalized first letter)
                label_id = LABEL_CLASS_MAP[answer['title']]
                subset = 'NA'
